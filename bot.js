...
const {Message, StringSession, Image, Video} = require('./ajfx/');
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.AFPLK == 'AJFX' || config.AFPLK == 'ajfx' || config.AFPLK == 'vava' || config.AFPLK == 'AJFX') {
        //thanks to afnanplk
        console.log(
            chalk.green.bold('thanks for watching -key cofirmed-')
        );
         }
         else if (config.AFPLK !== 'AJFX' || config.AFPLK !== 'ajfx' || config.AFPLK !== 'vava' || config.AFPLK !== 'AJFX') {
         console.log(
        ...
        console.log(
            chalk.green.bold('ᴀᴊғx ᴡᴏʀᴋɪɴɢ ' + config.WORKTYPE + ' ɴᴏᴡ 💞'));
          // thanks to afnanplk
	...
	var getGMTh = new Date().getHours()
	    ...
	    while (getGMTh == 7 && getGMTm == 01) {
            return await conn.sendMessage(conn.user.jid, img, MessageType.image, { mimetype: Mimetype.jpg, caption: '*[ DAILY ANNOUNCEMENTS AJFX ]*\n\n' + announce});
...
*/
                        ...
                        try {
                            ...
                            if (config.NOLOG === 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ ᴀᴊғx ~______~*' +
